...
const OUT = "C:/Users/Vicke/OneDrive/Desktop/Python/A_COH_10/Rishwanth/mediquick-ai/MediQuickAI_KnowledgeTransfer.docx";
...
const content = [
  ...
  center([new TextRun({ text: "An onboarding guide to the multimodal medical triage assistant  ·  June 2026", size: 18, color: MUT })], { spacing: { after: 60 } }),
  ...
  H1("2. How It Works"),
  P("The application is built in clear layers. A request flows from the browser to the backend API, through a set of focused services, to the AI model and the database, and back. The table below lists each component and whether it is finished or still planned."),
  ...
  P("The repository separates the backend from the front end. The most important files are listed below."),
  table([3300, 5726], [
    ["Path", "Purpose"],
    ["backend/main.py", "FastAPI app, CORS, router wiring, startup"],
    ...
    ["backend/routers/", "auth, chat, sessions, upload, dashboard"],
  ...
  cap("Table: key files in the MediQuick AI repository."),

  H1("6. Current Status (≈88% Complete)"),
  P("The full application — backend, authentication, specialist routing, the React interface, the dashboard, and the evaluation — is finished and was tested live in the browser. Only final polish and handover documentation remain."),
  table([4513, 4513], [
    ["Done", "Remaining"],
    ["FastAPI backend + SQLite (User/Session/Message)", "Error-handling and empty-state polish"],
    ["JWT auth, login / register, user profiles", "Short automated backend test suite"],
    ["NVIDIA NIM client (text + vision)", "Responsive-layout review for small screens"],
    ["Prompt engine, gather→conclude, JSON parser", "Developer README and setup guide"],
    ["10 specialist agents + routing / handoff", ""],
    ["React UI: chat, triage card, sidebar, dashboard", ""],
    ["30-scenario evaluation — 100% accuracy", ""],
  ]),
  cap("Done-versus-remaining at the mid-semester milestone (≈88% complete)."),

  H1("7. How to Run It"),
  P("The application runs locally with the backend and front end side by side."),
  step("In the backend folder, create a virtual environment and install dependencies: pip install -r requirements.txt"),
  step("Create a file named .env (copy from .env.example) and set NVIDIA_NIM_API_KEY to your NVIDIA NIM key from build.nvidia.com (a JWT_SECRET is also set there)."),
  ...
  H1("9. Next Steps"),
  bullet("Harden error handling and empty / loading states across the interface."),
  bullet("Add a short automated test suite for the backend services."),
  bullet("Review the responsive layout for smaller screens."),
  bullet("Write a developer README and setup guide for handover."),
];
